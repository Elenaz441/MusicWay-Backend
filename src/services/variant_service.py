...
class VariantService:
    ...
    async def get_variants_by_block(self, block_id: Optional[UUID], role: str) -> List[VariantListResponse]:
        ...
        for block in blocks:
            ...
            for variant in variants:
                ...
                variant_dict['description'] = variant['student_description'] if role == 'Ученик' \
                    else variant['teacher_description']
    ...
    async def get_variant(self, variant_id: UUID, role: str) -> VariantResponse:
        ...
        variant = await self.variant_repo.find_one(
            ['id', 'name', 'student_description', 'teacher_description', 'demo_url', 'task_type_id'],
            {'id': variant_id}
        )
        if not variant:
